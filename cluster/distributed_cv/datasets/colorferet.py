...
def load_from_full_dataset(colorferet_full_dir):
    ...
    for image_dir, image_subdirs, files in os.walk(image_dir_root):
        ...
        if len(matches_coords) and len(matches_race):
            ...
            if image_orig is not None:
                ...
                w_scaled = int(w_orig * bounds_ratio)
                h_scaled = int(h_orig * bounds_ratio)
...
def load_from_small_dataset(colorferet_small_dir):
    ...
    for label_str in labels_str:
        label_num = labels_str[label_str]
        image_dir = os.path.join(colorferet_small_dir, label_str)
        ...
        labels_tmp = [label_num] * len(images_tmp)
...
if __name__ == "__main__":
    sys.exit('colorferet.py is a library.')